...
def iplotpoly(inputA):
    ...
    def outputSaveMessage(s):
        ...
        save_message = plt.text(save_message_xloc-0.14,
                                text_yloc, s, transform=ax.transAxes)
        return
    ...
    def createSaveTextBoxes(default_filename):
        ...
        ax_filename_textbox = fig.add_axes(
            [save_message_xloc, 0.94, 0.3, 0.03])
        filename_textbox = TextBox(
            ax_filename_textbox, 'Output filename:', default_filename)
        plt.draw()
    ...
    def saveCallback(event):
        ...
        if (filename == '' or filename.isspace()):
            ...
            outputSaveMessage(
                'Illegal filename. Reset filename to default. No file saved.')
            ...
            try:
                np.savetxt(filename, A, header='iplotpoly vertices',
                           comments='# ')
            except:
                outputSaveMessage('Unable to save to file ' +
                                  filename + '. Check write permissions.')
                plt.draw()
                return
    ...
    def addVertexCallback(event):
        ...
        if ((event.x < axLL[0]) or (event.x > axUR[0])):
            return
        if ((event.y < axLL[1]) or (event.y > axUR[1])):
            return
    ...
    def pickPointCallback(event):
        ...
        if (event.ind[0] >= 0) and (event.ind[0] < len(vX)):
            ...
            cid_moveCallback = fig.canvas.mpl_connect(
                'button_release_event', moveVertexCallback)
    ...
    def createButtons(left_pos, button_width, button_height):
        ...
        ax_subdivide_button = plt.axes(
            [left_pos, 0.8, button_width, button_height])
        ...
        ax_rotateCW_button = plt.axes(
            [left_pos, 0.7, button_width, button_height])
    ...
    if (jv < numv):
        ...
        cid_addVertexCallback = fig.canvas.mpl_connect(
            'button_press_event', addVertexCallback)
    else:
