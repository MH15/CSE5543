...
_.extend(Vector.prototype, Events, {
  ...
  rotate: function(Number2) {
    var cos2 = Math.cos(Number2);
    var sin2 = Math.sin(Number2);
    this.x = this.x * cos2 - this.y * sin2;
    this.y = this.x * sin2 + this.y * cos2;
    return this;
  }
});
var BoundProto = {
  ...
  rotate: function(Number2) {
    var cos2 = Math.cos(Number2);
    var sin2 = Math.sin(Number2);
    this._x = this._x * cos2 - this._y * sin2;
    this._y = this._x * sin2 + this._y * cos2;
    return this;
  }
};
var xgs = {
...
_.extend(Anchor, {
  AppendCurveProperties: function(anchor2) {
    ...
    anchor2.controls = {};
    anchor2.controls.left = new Vector(0, 0);
    anchor2.controls.right = new Vector(0, 0);
  },
  MakeObservable: function(object) {
...
var getAnchorsFromArcData = function(center, xAxisRotation, rx, ry, ts, td, ccw) {
  ...
  for (var i = 0; i < resolution; i++) {
    ...
    var anchor2 = new Anchor(x, y);
    Anchor.AppendCurveProperties(anchor2);
...
_.extend(Group.prototype, Shape.prototype, {
  ...
  getBoundingClientRect: function(shallow) {
    ...
    var regex2 = /texture|gradient/i;
    matrix = shallow ? this._matrix : getComputedMatrix(this);
    for (var i = 0; i < this.children.length; i++) {
      var child = this.children[i];
      if (!child.visible || regex2.test(child._renderer.type)) {
        continue;
      }
...
var emptyArray = [];
var TWO_PI$5 = Math.PI * 2, max$2 = Math.max, min$2 = Math.min, abs = Math.abs, sin$4 = Math.sin, cos$4 = Math.cos, acos = Math.acos, sqrt = Math.sqrt;
...
var canvas = {
  ...
  path: {
    render: function(ctx, forced, parentClipped) {
      var matrix, stroke, linewidth, fill, opacity, visible, cap, join, miter, closed, commands, length, last, next, prev, a, b, c, d, ux, uy, vx, vy, ar, bl, br, cl, x, y, mask, clip, defaultMatrix, isOffset, dashes, po;
  ...
  renderSvgArcCommand: function(ctx, ax, ay, rx, ry, largeArcFlag, sweepFlag, xAxisRotation, x, y) {
    ...
    var cxp = q * rx * y1p / ry;
    var cyp = -q * ry * x1p / rx;
    var cx = cos$4(xAxisRotation) * cxp - sin$4(xAxisRotation) * cyp + (ax + x) / 2;
    var cy = sin$4(xAxisRotation) * cxp + cos$4(xAxisRotation) * cyp + (ay + y) / 2;
    var startAngle = svgAngle(1, 0, (x1p - cxp) / rx, (y1p - cyp) / ry);
    var delta = svgAngle((x1p - cxp) / rx, (y1p - cyp) / ry, (-x1p - cxp) / rx, (-y1p - cyp) / ry) % TWO_PI$5;
...
function renderArcEstimate(ctx, ox, oy, rx, ry, startAngle, endAngle, clockwise, xAxisRotation) {
  ...
  for (var i = 0; i < Constants.Resolution; i++) {
    ...
    if (xAxisRotation !== 0) {
      var cos2 = Math.cos(xAxisRotation);
      var sin2 = Math.sin(xAxisRotation);
      var tx = x - ox;
      var ty = y - oy;
      x = tx * cos2 - ty * sin2 + ox;
      y = tx * sin2 + ty * cos2 + oy;
    }
    ctx.lineTo(x, y);
  }
}
  ...
  getRequestAnimationFrame: function() {
    ...
    if (!request) {
      for (var i = 0; i < vendors.length; i++) {
        ...
        cancel = root$1[vendors[i] + "CancelAnimationFrame"] || root$1[vendors[i] + "CancelRequestAnimationFrame"] || cancel;
      }
...
function RadialGradient(cx, cy, r, stops, fx, fy) {
  ...
  this.center = new Vector().bind(Events.Types.change, function() {
    this._flagCenter = true;
  }.bind(this));
  this.radius = typeof r === "number" ? r : 20;
  this.focal = new Vector().bind(Events.Types.change, function() {
    ...
    switch (key) {
      ...
      case "y":
        ...
        if (value.match("[a-z%]$") && !value.endsWith("px")) {
          var error = new TwoError("only pixel values are supported with the " + key + " attribute.");
...
var read = {
  ...
  path: function(node, parentStyles) {
    ...
    path = new Path(points, closed, void 0, true).noStroke();
    path.fill = "black";
  ...
  rect: function(node, parentStyles) {
    ...
    var w2 = width / 2;
    var h2 = height / 2;
    var rect = new Rectangle(0, 0, width, height).noStroke();
    rect.fill = "black";
  ...
  "rounded-rect": function(node, parentStyles) {
    ...
    var rect = new RoundedRectangle(0, 0, width, height, radius).noStroke();
    rect.fill = "black";
    applySvgAttributes.call(this, node, rect, parentStyles);
    rect.translation.x += w2;
    rect.translation.y += h2;
  ...
  lineargradient: function(node, parentStyles) {
    ...
    var gradient = new LinearGradient(x1 - ox, y1 - oy, x2 - ox, y2 - oy, stops);
  ...
  radialgradient: function(node, parentStyles) {
    ...
    var gradient = new RadialGradient(cx - ox, cy - oy, r, stops, fx - ox, fy - oy);
    applySvgAttributes.call(this, node, gradient, parentStyles);
    return gradient;
  },
...
var multiplyMatrix = Matrix.Multiply, identity = [1, 0, 0, 0, 1, 0, 0, 0, 1], transformation = new NumArray(9), CanvasUtils = Renderer$2.Utils;
var webgl = {
  isHidden: /(undefined|none|transparent)/i,
...
function Renderer(params) {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, new NumArray([
    0,
    0,
    1,
    0,
...
_.extend(Two.prototype, Events, {
  ...
  makeCurve: function(p) {
    ...
    var curve = new Path(points, !(typeof last === "boolean" ? last : void 0), true);
  ...
  makePath: function(p) {
    ...
    var last = arguments[l - 1];
    var path = new Path(points, !(typeof last === "boolean" ? last : void 0));
  ...
  load: function(text, callback) {
    ...
    var attach = function(data) {
      ...
      if (typeof callback === "function") {
        var svg2 = dom.temp.children.length <= 1 ? dom.temp.children[0] : dom.temp.children;
        callback(group, svg2);
      }
    }.bind(this);
    if (/.*\.svg/ig.test(text)) {
